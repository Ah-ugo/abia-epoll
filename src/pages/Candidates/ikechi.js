...
import Ikechiimg from "../../sources/ikechi.webp";
...
export const Ikechi = () => {
  const [Ikechistate, setIkechi] = useState(0);
  const [Ikechivote, setIkechivote] = useState(0);
  ...
  const AddVote = () => {
    Endpoint.post("/Ikechi", { votes: Ikechivote }).then((res) => {
      console.log(res);
      if (res.status === 200) {
        setShowModal(true);
      } else {
  ...
  useEffect(() => {
    Endpoint.get("/Ikechi").then((res) => {
      setIkechi(res.data.countInfo.count);
      console.log(res);
    });
  }, [AddVote]);
  return (
      ...
      <Container>
        <center>
          <Card
            ...
            <CardImg src={Ikechiimg} style={{ maxHeight: "300px" }} />
            <Card.Title>High Chief Ikechi Emenike</Card.Title>
            <Card.Subtitle>Party: APC</Card.Subtitle>
            <Card.Body bg="secondary">
              <Card.Text>Total Votes: {Ikechistate}</Card.Text>
              <Button
