...
import Nanaimg from "../../sources/nana.webp";
...
export const Nana = () => {
  const [Nanastate, setNana] = useState(0);
  const [Nanavote, setNanavote] = useState(0);
  ...
  const AddVote = () => {
    Endpoint.post("/Nana", { votes: Nanavote }).then((res) => {
      console.log(res);
      if (res.status === 200) {
        setShowModal(true);
      } else {
  ...
  useEffect(() => {
    Endpoint.get("/Nana").then((res) => {
      setNana(res.data.countInfo.count);
      console.log(res);
    });
  }, [AddVote]);
  return (
      ...
      <Container>
        <center>
          <Card
            ...
            <CardImg src={Nanaimg} style={{ maxHeight: "300px" }} />
            <Card.Title>Engr. Enyinnaya Nwafor</Card.Title>
            <Card.Subtitle>Party: YPP</Card.Subtitle>
            <Card.Body bg="secondary">
              <Card.Text>Total Votes: {Nanastate}</Card.Text>
              <Button
